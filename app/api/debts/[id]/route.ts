...
export async function PATCH(
  ...
  if (counterpart_name !== undefined) {
    if (typeof counterpart_name !== 'string' || counterpart_name.trim() === '') {
      return NextResponse.json({ error: 'counterpart_name cannot be empty' }, { status: 400 })
    }
  ...
  if (due_date !== undefined) {
    if (due_date !== null && isNaN(Date.parse(due_date))) {
      return NextResponse.json({ error: 'due_date must be a valid date or null' }, { status: 400 })
    }
    updates.due_date = due_date
  }

  if (settled_at !== undefined) {
    if (settled_at !== null && isNaN(Date.parse(settled_at))) {
      return NextResponse.json({ error: 'settled_at must be a valid date or null' }, { status: 400 })
    }
